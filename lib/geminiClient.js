...
// Removed: import { GoogleGenerativeAI } from '@google/generative-ai'; // No longer using the client library directly
...
/**
 * Generates an image using the Google Imagen API with a rotating API key and direct fetch.
 ...
 * @throws {Error} If all API keys fail.
 */
export async function getGeminiImageResponse(imagePrompt) {
  const KEYS = getProcessedApiKeys();
  let lastError = null;
  ...
  for (let i = 0; i < KEYS.length; i++) {
    ...
    try {
      ...
      if (result.predictions && result.predictions.length > 0 && result.predictions[0].bytesBase64Encoded) {
        ...
        throw new Error("Imagen API did not return a valid image in the expected format.");
      }

    } catch (err) {
      lastError = err;
      const keyPrefix = apiKey ? apiKey.substring(0, 5) + '...' : 'N/A';
      console.warn(`Imagen API error with key ${keyPrefix}. Trying next key. Error: ${err.message}`);
      continue; // Always try the next key in the rotation
    }
  }
